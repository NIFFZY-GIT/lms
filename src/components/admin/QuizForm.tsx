...
import { Question } from '@/types'; // This form manages a Question

// --- Schema and Type for a QUESTION ---
export const questionSchema = z.object({
    ...
    answers: z.array(z.object({
        ...
        isCorrect: z.boolean(),
    })).length(4, "Must provide 4 answers.").refine(
...
interface QuestionFormProps {
    ...
    initialData?: Question; // Expects a Question object for editing
    isPending: boolean;
    onCancel: () => void;
}

export function QuestionForm({ onSubmit, initialData, isPending, onCancel }: QuestionFormProps) {
    // --- The useForm hook is correctly typed with QuestionFormData ---
    const { register, handleSubmit, control, setValue, getValues, formState: { errors } } = useForm<QuestionFormData>({
        resolver: zodResolver(questionSchema),
        // This correctly sets default values for both creating and editing
        defaultValues: initialData ? {
            questionText: initialData.questionText,
            answers: initialData.answers.map(a => ({ id: a.id, answerText: a.answerText, isCorrect: a.isCorrect }))
        } : {
            questionText: '',
            answers: [
