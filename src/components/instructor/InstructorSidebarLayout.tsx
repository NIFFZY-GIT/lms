  ...
  return (
    <Link
      ...
      className={`flex items-center px-3 py-2.5 text-sm font-medium rounded-md transition-colors ${
        isActive ? 'bg-indigo-600 text-white' : 'text-indigo-200 hover:bg-indigo-700 hover:text-white'
      }`}
    >
...
export default function InstructorSidebarLayout({ userName, children }: { userName: string; children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      {/* Sidebar */}
      <aside
        className={`fixed inset-y-0 left-0 bg-indigo-800 text-white w-64 p-4 z-50 transform transition-transform duration-300 ease-in-out lg:relative lg:translate-x-0 ${
        ...
        <header className="lg:hidden sticky top-0 bg-white/80 backdrop-blur-sm shadow-sm z-30">
          <div className="flex items-center justify-between p-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="bg-indigo-600 text-white rounded-full h-8 w-8 flex items-center justify-center text-sm font-bold">
